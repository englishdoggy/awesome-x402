...
- [Agent Café](https://402.coffee) - Test and certify your x402 **client** (the payer), not the seller: point your paying agent at a live Base-mainnet endpoint, it pays real USDC, and you get back a public certificate + README badge + a checkable credential of exactly what it did. Two behavioural tests — **scam-resistance** (does it refuse a deliberately over-priced order?) and **recipient-awareness** (does it notice when the payment recipient is swapped?). Counterparties verify any agent in one call: [`/verify?wallet=`](https://api.402.coffee/verify) with 30-day validity, plus a public [board](https://api.402.coffee/board). Registered ERC-8004 agent on Base. Free machine-readable menu at [/inspect](https://api.402.coffee/inspect). [Docs](https://api.402.coffee/docs) | [Examples](https://github.com/englishdoggy/agentcafe-examples)
